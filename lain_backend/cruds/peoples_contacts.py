...
async def update(db: Database, people_ids: List[int], contact_id: int) -> None:
    await db.execute(
        model.delete().where(
            and_(model.c.contact_id == contact_id, not_(model.c.people_id.in_(people_ids)),)
        )
    )

    for oid in people_ids:
        if not await db.execute(
            model.exists().where(and_(model.c.contact_id == contact_id, model.c.people_id == oid,))
        ):
            await db.execute(model.insert().values(contact_id=contact_id, people_id=oid))
