...
async def test_get_all(db):
    ...
    for i in range(2):
        for key in dict(db_protocols[i]):
            assert db_protocols[i][key] == protocols[i][key]
...
async def test_get_one(db):
    ...
    for key in dict(protocol):
        assert db_protocol[key] == protocol[key]
...
async def test_exists(db):
    ...
    assert await crud.exists(db=db, name=protocol["name"])
...
async def test_check(db):
    ...
    assert await crud.check(db=db, protocol_id=protocol["id"])
